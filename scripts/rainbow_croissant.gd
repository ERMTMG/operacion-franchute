...
func split_into_rainbow_chunks(number: int, speed: float, spin: float, size: float, hp: int) -> void:
	...
	for i in range(number):
		...
		if ithEnemy is RainbowCroissantChunk:
			...
			ithEnemy.colorHue = fmod(initialColor + i * colorSpread, 1.0)
...
func die() -> void:
	if !is_queued_for_deletion():
		...
		Global.create_vfx(colorFlash, global_position + Vector2.from_angle(randf() * TAU) * 50 * randf())
		Global.create_vfx(colorFlash, global_position + Vector2.from_angle(randf() * TAU) * 50 * randf())
		Global.create_vfx(colorFlash, global_position + Vector2.from_angle(randf() * TAU) * 50 * randf())
